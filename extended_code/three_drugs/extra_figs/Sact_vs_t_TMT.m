...
TMT_tot=[0 1 5 10 25 50 100]/10;

options = odeset('Mass',M, 'MassSingular','yes', 'RelTol',1e-2, 'AbsTol',1e-2);
set(gcf,'position',[100,100,1200,700])
newcolors = {'#008000',' #FFC300 ', '#FF5733 ','#C70039','#900C3F','#581845','#000000','#DAF7A6'};

%%%%%%%%%%%%%%%%%%%%%%% Generate activated SUB (pSUB) vs t FIGURE for different TMT values
...
for k = 1:4
    ...
    for j = 1:length(TMT_tot)
        TMT_in=TMT_tot(j);
        y0(30)=TMT_in;
        [t,y] = ode15s(@(t,y) mapk_cascade_DAE(y, BRAF_tot(k), ATP_tot(k), DBF_tot, TMT_in, SCH_tot), tspan, y0, options);
        ysubact = y(:,41)/1.2;
        hold on
