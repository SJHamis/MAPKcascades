...
timepoint_concentrations=zeros(length(DBF_range),length(SCH_range));
    ...
    for col = 1:3
            for i_SCH = 1:length(SCH_range)
                for i_DBF = 1:length(DBF_range)
                    timepoint_concentrations(i_SCH,i_DBF) = generate_concentration_data_sact(userinput_BRAF_tot(row), userinput_ATP_tot(row),...
                        DBF_range(i_DBF), TMT_in, SCH_range(i_SCH), tend(col));
                end
            end
