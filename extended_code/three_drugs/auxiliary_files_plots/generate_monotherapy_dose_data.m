...
while monotherapy_points(1,1) == 0
    ...
    if yoact > IC_in && stage==1
        previous_yoact = yoact;
        previous_TMT = TMT_in;
        TMT_in = TMT_in+1;
    elseif yoact <= IC_in && stage==1
        TMT_in=previous_TMT;
        stage=2;

    elseif yoact > IC_in && stage==2
        previous_yoact = yoact;
        previous_TMT = TMT_in;
        TMT_in = TMT_in+0.1;
    elseif yoact <= IC_in && stage==2
    ...
    elseif yoact > IC_in && stage==3
        previous_yoact = yoact;
        previous_TMT = TMT_in;
        TMT_in = TMT_in+0.01;
    elseif yoact <= IC_in && stage==3
        if abs(previous_yoact-0.25) <= abs(yoact-0.25)
